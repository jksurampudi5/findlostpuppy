...
graph TD
    ...
    D2 --> E[Future Cloud Backend: Firebase / Cloud Run]
...
3. **Offline-First & Extensibility**:
   - Built on a repository service abstraction (`storageService.ts`), allowing seamless substitution of local storage with Firebase or GraphQL backend APIs without modifying UI components.
